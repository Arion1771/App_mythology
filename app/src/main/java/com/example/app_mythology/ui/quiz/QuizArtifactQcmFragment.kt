...
/** Quiz QCM Artéfacts : 4 choix, un seul essai, écran de résultat par question. */
class QuizArtifactQcmFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fun bindQuestion(artifacts: List<ArtifactEntity>, index: Int) {
            ...
            tvClue.text      = a.clue?.takeIf { it.isNotBlank() }?.let { "Indice : $it" } ?: "Indice : —"
            tvBadge.text     = a.difficulty.toString()
            tvBadge.setBackgroundResource(difficultyBadgeRes(a.difficulty))
        }

        viewModel.qcmArtifacts.observe(viewLifecycleOwner) { artifacts ->
            rebuildDots(artifacts.size)
            viewModel.qcmIndex.observe(viewLifecycleOwner) { index ->
                if (viewModel.qcmFinished.value != true) bindQuestion(artifacts, index)
            }
        }
        ...
        viewModel.qcmAnswerRevealed.observe(viewLifecycleOwner) { revealed ->
            if (revealed) {
                findNavController().navigate(R.id.action_qcmArtifact_to_qcmArtifactResult)
            }
        }
